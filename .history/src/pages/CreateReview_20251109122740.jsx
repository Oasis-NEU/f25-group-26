...
const CreateReview = () => {
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      setImageFile(file);
      // Create instant preview with object URL (no conversion needed)
  ...
  // In CreateReview.jsx, update the uploadPhotoToStorage function:
...
const uploadPhotoToStorage = async (file, reviewId) => {
  try {
    ...
    // Upload to Supabase Storage (make sure bucket name is correct)
    const { data: uploadData, error: uploadError } = await supabase.storage
      .from('ReviewPhotos') // Make sure this bucket exists in Supabase
    ...
    if (photoError) {
      ...
      console.log('Photo saved successfully:', insertedPhoto);
    }
  } catch (err) {
